      ...
      <div class="col-sm-10">
         <select class="form-control" id="sel1" name="month" onBlur="doCalc(this.form)" required>
          ...
          <?php
								...
								$result = $con->query("SELECT * FROM daily group by month  order by id DESC") or die(mysqli_error($con));
      ...
      <div class="col-sm-10">
        ...
        <select class="form-control" id="sel1" name="year" onBlur="doCalc(this.form)" required>
          ...
          <?php
								...
								$result = $con->query("SELECT * FROM daily group by year order by id DESC") or die(mysqli_error($con));
				while($bu=$result->fetch_assoc()){
								?>
...
?>
  ...
  <table class="table table-bordered sortableTable responsive-table" style="line-height:1.7">
   ...
   $dms=$con->query("SELECT * FROM daily where month='$m' and year='$y'   GROUP BY date ") or die(mysqli_error($con));
   $i=1;
...
<td><?php echo $i++; ?></td>
     ...
     <td><a href="?viewing_sales&date=<?php echo $bums['date']; ?>&sect=<?php echo $bums['receiver']; ?>" >View  <?php echo $bums['date']; ?>  Sales Record</a>                       </tr>
