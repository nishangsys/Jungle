      ...
      <div class="col-sm-10">
         <select class="form-control" id="sel1" name="month" onBlur="doCalc(this.form)" required>
          ...
          <?php
								...
								$result = $con->query("SELECT * FROM truck group by year order by id DESC") or die(mysqli_error($con));
      ...
      <div class="col-sm-10">
        ...
        <select class="form-control" id="sel1" name="year" onBlur="doCalc(this.form)" required>
          ...
          <?php
								...
								$result = $con->query("SELECT * FROM truck group by status order by id DESC") or die(mysqli_error($con));
				while($bu=$result->fetch_assoc()){
								?>
      ...
      <div class="col-sm-10">
        ...
        <select class="form-control" id="sel1" name="sector" onBlur="doCalc(this.form)" required>
          ...
          <?php
								...
								$result = $con->query("SELECT * FROM truck,sectors where sectors.area=truck.receiver group by truck.receiver order by truck.receiver DESC") or die(mysqli_error($con));
				while($bu=$result->fetch_assoc()){
								?>
...
?>
  ...
  <table class="table table-bordered sortableTable responsive-table" style="line-height:1.7">
   ...
   $dms=$con->query("SELECT * FROM truck where year='$m' and status='$y' and receiver='$s' GROUP BY month ") or die(mysqli_error($con));
   $i=1;
...
<td><?php echo $i++; ?></td>
     ...
     <td><a href="?view_supplies&date=<?php echo $bums['month']; ?>&sect=<?php echo $bums['receiver']; ?>" target="new" style="font-family:'Arial Black', Gadget, sans-serif; color:#f00; ">View  <?php echo $bums['month']; ?> Supply and undo</a>
                                   </tr>
