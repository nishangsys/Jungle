...
if(isset($_POST['ok'])){	
	...
	$dept=$_POST['name'];
	$gh12=$con->query("DELETE FROM sector WHERE name='$dept' ") or die(mysqli_error($con));
	
	
	$gh=$con->query("INSERT INTO sector set name='$dept' ") or die(mysqli_error($con));
...
if(isset($_GET['delete'])){
	$gh=$con->query("delete from sector where id='".$_GET['delete']."' ") or die(mysqli_error($con));
...
<?php echo $message; ?>
  ...
  <form class="form-horizontal" role="form" action="" method="post" style="margin-top:30px">
    <div class="form-group">
      ...
      <div class="col-sm-10">
        <input type="text" class="form-control" id="email" placeholder="Sector" name="name">
      </div>
    </div>
    ...
    <div class="form-group">        
      <div class="col-sm-offset-2 col-sm-10">
        <button type="submit" class="btn btn-info" name="ok">SAVE</button>
      </div>
    </div>
  </form>
</div>
...
<?php $d=$con->query("SELECT * FROM sector order by name   ") or die(mysqli_error($con));
$i=1;
?>
