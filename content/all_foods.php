...
?>
 ...
 <thead>
   ...
   <?php $d=$con->query("SELECT * FROM foods order by pro_id DESC  ") or die(mysqli_error($con));
$i=1;
?>
      ...
      <tr>
        ...
        <td><?php echo $name=$df['product']; ?></td>
         ...
         <td><?php echo $df['serial']; ?></td>
 <td><?php echo $df['price']; ?></td>
 <td><a href="?adding_foods&link= MAIN COURSE &type=MAIN COURSE&delete=<?php echo $df['pro_id']; ?>"><button class="btn btn-danger"><i class="icon-cross icon-white"></i> DELETE</button></a></td>
...
if(isset($_GET['delete'])){
	 $id=$_GET['delete'];
	  $dfG=$con->query("DELETE FROM foods where pro_id='$id'") or die(mysqli_error($con));
	   ...
	   $message= "<div class='alert alert-success'>Item Successfully Deleted. Thank You</div>";
	   echo '<meta http-equiv="Refresh" content="0; url=all_foods.php ">';
