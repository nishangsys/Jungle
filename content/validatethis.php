	...
	for($i=1; $i<=$amt; $i++) {
	 ...
	 $_POST['sentothem$i']=$_POST["qty$i"];
	  ...
	  $paidto=$_SESSION['user_name'];
	   ...
	   $upbn=mysql_query("UPDATE `warehouse` SET `qty`= '".$_POST["lefts$i"]."'   WHERE `id` = '".$_POST["pro_id$i"]."' 		  ") or die(mysql_error());
			
		
		 $update_disburse =mysql_query  ("UPDATE `disburse` SET `taken`= '2',units='".$_POST["discribe$i"]."' WHERE `stock` = '".$_POST["product$i"]."'  and taken!='2'  ") or die(mysql_error());
	           		
	
	echo "<script>alert('Thank for your Patience . ')</script>";
echo '<meta http-equiv="Refresh" content="0; url=../POS/content/close.php">';
...
<h1 style="background:#088389; text-align:center; font-size:24px; padding:20px 0px; color:#fff;"><a href="receiving.php?receiving=Admin&branch=BUEA&supp=general" style="color:#FF0; text-decoration:none">&infin; UP BY ONE LEVEL</a> Stock Validation for <span style="color:#ff0">  Super Market  <br><span style="color:#fff">Qty Supplied: <span style="color:#FF0"><?php echo ($qty); ?> </span>Worth of Distributed Product is </span> <?php echo number_format($cost); ?> Frs</span></h1>
    ...
    ?>
            ...
            <tr>
                ...
                <td><input type="text" name="product<?php echo $i; ?>" value="<?php echo $row['stock']; ?>" class="input" required style="width:190px" readonly   /></td>
                  <td><input type="text" name="cost<?php echo $i; ?>" value="<?php $df=mysql_query("SELECT * FROM warehouse where name='".$row['stock']."'  ") or die(mysql_error());
				  ...
				  ?>" class="input" style="width:130px" readonly  /></td>
				   ...
				   $s=mysql_query("SELECT *  from warehouse where name='".$row['stock']."' 
			    ") or die(mysql_error());
				while($d=mysql_fetch_assoc($s)){
					$c=$d['cost'];
					
				}   echo $row['SUM(current)']*$c; ?>" class="input" style="width:60px" readonly  /></td>
