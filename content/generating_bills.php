...
//////////////////////////////////////TAXABLE SALARIES SET ////////////////////////////////////////////////////
 ...
 $query= $con->query("SELECT * FROM store  where sector like '%$zone%' order  by name") or die(mysqli_error($con)); // Select from the table
...
if(isset($_POST['SubmitUpdate'])) {
	...
	$cate=$_GET['cate'];
	$qu	=$con->query("SELECT COUNT(name) as totcou FROM store  where sector like '%$zone%'  ") or die(mysqli_error($con)); // Select from the table
...
/////else run qury
	...
	///////insert records into the history table
			...
			///////update stock
			$stock =$con->query( "UPDATE store set qty='".$_POST["left$i"]."' where id='".$_POST["pid$i"]."'") or die(mysqli_error($con));
...
<div class="as_wrapper">
  ...
  <td><input type="text" name="qty<?php echo $i; ?>" value="<?php echo $row['qty']; ?>" class="input" readonly  /></td>
