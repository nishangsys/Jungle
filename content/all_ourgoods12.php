...
?>
      ...
      <tr>
         ...
         <td style="color:#f00"><?php 
		  $dio=$con->query("SELECT * FROM sectors where area='".$df['area']."'  ") or die(mysqli_error($con));
$i=1;
while($buio=$dio->fetch_assoc()){
	echo $branch=$buio['name'];
}	
  ?></td>
...
<td><a href="?record_goods&link=Adding Goods&add_good&delete=<?php echo $df['id']; ?>"><button class="btn btn-danger"><i class="icon-cross icon-white"></i> DELETE</button></a></td>
...
if(isset($_GET['delete'])){
	 $id=$_GET['delete'];
	  $dfG=$con->query("DELETE FROM finals where id='$id'") or die(mysqli_error($con));
	   ...
	   echo '<meta http-equiv="Refresh" content="0; url=all_ourgoods12.php ">';
