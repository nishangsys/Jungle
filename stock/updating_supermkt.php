...
if(isset($_GET['delete'])){
  ...
  if(isset($_POST['Update'])){
	  ...
	  $Dis=$_POST['disc'];
	   ...
	   $qtty=$_POST['qtty'];
	 $id=$_GET['update'];
	  $dfGu=$con->query("UPDATE supermkt SET name='$shape',disc='$Dis',cost='$C',sp='$S',qty='$qtty' where id='$id'") or die(mysqli_error($con));
...
$qty=$_POST['qty'];
$sp=$_POST['sp'];
//$df=$con->query("DELETE FROM supermkt where name='$shape' and disc='$disc' and branch='$branch'") or die(mysqli_error($con));
$o=$con->query("SELECT * FROM supermkt WHERE name='$shape' and disc='$disc' and branch='$branch'") or die(mysqli_error($con));
while($cc=$o->fetch_assoc()){
	$av=$cc['qty'];
	$nqty=$av+$qty;
}
if(mysqli_num_rows($o)>0){
	$dfGu=$con->query("UPDATE supermkt SET qty='$nqty',cost='$cost',name='$shape' WHERE name='$shape' AND disc='$disc'") or die(mysqli_error($con));
	   $message= "<div class='alert alert-success'>Item Successfully Updated. Thank You</div>";
}
else {
...
$do=$con->query("INSERT INTO supermkt SET name='$shape',disc='$disc',branch='$branch',qty='0',cost='$cost',sp='$sp'  ") or die(mysqli_error($con));
...
if(isset($_GET['delete'])){
  ...
  /////////////for updates
  $doU=$con->query("SELECT * FROM supermkt WHERE id='".$_GET['add_good']."'") or die(mysqli_error($con));
  while($nam=$doU->fetch_assoc()){
	 $sha=$nam['name'];
	  $dis=$nam['disc'];
	  $cp=$nam['cost'];
  ...
  if(isset($_POST['Update'])){
	 ...
	 $id=$_GET['update'];
	 $qtty=$_POST['qtty'];
	  $dfGu12=$con->query("UPDATE supermkt SET name='$shape',disc='$Dis',cost='$C',sp='$S',qty='$qtty' where id='$idd'") or die(mysqli_error($con));
	   $message= "<div class='alert alert-success'>Item Successfully Updated. Thank You</div>";
  ...
  <div class="form-group">
    ...
    <div class="col-sm-10"> 
      <input type="text" required class="form-control" id="pwd" value="<?php echo $sha; ?>" name="name" placeholder="Product"  style="background:#FF9; border:2px solid#f00" readonly="readonly" >
    </div>
  </div>
   ...
   <div class="form-group">
    ...
    <label class="control-label col-sm-2" for="pwd"></label>
    <div class="col-sm-10"> 
      <input type="text" required class="form-control" id="pwd" value="<?php echo $qty; ?>" name="qtty" placeholder="Qty:" style="width:120px;background:#FF9; border:2px solid#f00" readonly="readonly"  >
    </div>
  </div>  
...
<button class="btn btn-primary"><i class="icon-edit icon-white"></i> UPDATE Price</button>
