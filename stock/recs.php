...
while($cx=mysql_fetch_assoc($ad)){
   ...
   $result=mysql_query("SELECT product,category,SUM(qty),price,SUM(qty*price),ids from basket where tab='".$tale."' and qty>0  group by product  ");
$O=1; 
...
<?php
$result=mysql_query("SELECT SUM(price*qty) as total,tab from basket where tab='".$tale."'  GROUP BY tab  ");
...
<?php
$result=mysql_query("SELECT * from daily where cust_id='".$_GET['roll']."' order by id ASC LIMIT 1");
while ($row=mysql_fetch_assoc($result)){ 
...
<TR>
<TD>Amt Paid Today</TD><td><?php 
$date=date('d-m-Y');
$a=$con->query("SELECT SUM(rec),owed from daily where date='".$date."' and cust_id='".$_GET['roll']."' GROUP BY date ");
while ($b=$a->fetch_assoc()){ 
echo number_format($b['SUM(rec)']) ;
}
?></td></TR>

<TR>
<TD>Amount Paid sofar</TD><td><?php 
$a=$con->query("SELECT SUM(rec),owed from daily where cust_id='".$_GET['roll']."' GROUP BY cust_id ");
while ($b=$a->fetch_assoc()){ 
echo number_format($b['SUM(rec)']) ;
}
?></td></TR>
