...
////service
    $df=$con->query("SELECT * from basket,users where basket.tab='".$table_num."' and ids='".$command_name."' and basket.agent=users.id and basket.printed='0' and basket.area='$area' and basket.qty>0 group by basket.tab  ") or die(mysqli_error($con));
	while($dff=$df->fetch_assoc()){
		$service=$dff['user_name'];
	}
 
   $result=$con->query("SELECT product,category,SUM(qty),price,SUM(qty*price),ids from basket where tab='".$table_num."' and ids='".$command_name."' and printed='0' and area='$area' and qty>0 group by product  ")or die(mysqli_error($con));
$O=1; 
echo $hhj=$result->num_rows;
		 ...
		 <div class="page">
          ...
          <h1 style="font-size:14px; text-align:center; font-family:Arial, Helvetica, sans-serif; margin-top:5px"><?php echo $clients; ?></h1>
           ...
           Service : <?php echo $service; ?>
		    ...
		    <h1 style="font-size:14px; text-align:center; font-family:Arial, Helvetica, sans-serif; margin-top:-5px; border:1px solid#000"> Receipt  Num: <?PHP echo $nl;  ?> </span><br>
            SALES ON CREDIT
...
<TR>
<TD>Amount Owed</TD><td><?php echo number_format($row['owed']) ?></td></TR>
</table>
...
$insert=$con->query("INSERT INTO history SET agent='$level',date='$date',num='$nl',sector='$arm',amt='$paids',name='$yn',owed='$owed' ") or die(mysqli_error($con));
