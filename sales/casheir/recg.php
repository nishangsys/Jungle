<?php
echo '<meta http-equiv="Refresh" content="3; url=../casheir/index.php?area='.$_GET['area'].' ">';
...
$tale=$_GET['roll'];
  $serial;
   ...
   $result=mysql_query("SELECT product,category,SUM(qty),price,SUM(qty*price),ids from basket where tab='".$tale."' and agent='$paidto' and printed!='2' and qty>0 group by product  ");
$O=1; 
...
<?php
$result=mysql_query("SELECT SUM(price*qty) as total,tab from basket where tab='".$tale."' and printed!='2' and agent='$paidto'  GROUP BY tab  ");
...
<?php
  $first=mysql_query("UPDATE basket set printed='2',froms='$nl',agent='$level' where printed!='2' and agent='$paidto'  ") or die(mysql_error());
...
echo '<meta http-equiv="Refresh" content="0; url=../casheir/index.php?area='.$_GET['area'].'">';
 ?>
