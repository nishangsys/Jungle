...
$area=$_GET['area'];
  $serial;
    $dms=$con->query("SELECT * FROM sectors WHERE area='".$_GET['area']."' ") or die(mysqli_error($con));
...
$C=$con->query("SELECT * FROM overtime where id='1' order by id DESC LIMIT 1") or die(mysqli_error($con));
   ...
   $result=$con->query("SELECT name as product,qty,sp as price from finals where area='".$_REQUEST['area']."' and name!='' order by name   ") or die(mysqli_error($con));
$O=1; 
...
?>
		 ...
		 <div class="page">
          ...
          <h1 style="font-size:14px; text-align:center; font-family:Arial, Helvetica, sans-serif; margin-top:5px"><?php echo $clients; ?></h1>
           ...
           <br>
          <?php echo $sector;  ?> STOCK FOR  : <?php echo $cur_date; ?></h1>
