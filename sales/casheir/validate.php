...
$query 	= mysql_query("SELECT * FROM basket  where tab='".$table_num."' and ids='".$command_name."'  and status= '5' ORDER BY id ASC") or die(mysql_error()); // Select from the table
...
if(isset($_POST['SubmitUpdate'])) {
	...
	$_POST = array_map("ucwords", $_POST);
	 ...
	 $am=abs($change);
	 if($change<0){
		 echo "<script>alert('Underpaying By $am')</script>";
	 }
		...
		$acv=$con->query("INSERT INTO daily set cust_id='$who',room='',amt='$amt_owed',reason=' Bill',qty='1',price='$totalbill',total='$totalbill',
			owed='$amt_owed',date='$cur_date',month='$month',year='$year',area='".$_GET['area']."',time='$time',paidto='$userm',purpose='Bills' ,idds='',discount='$discoun',staffname='$name',rec='$totalbill',paidtou='$am' ") or die(mysqli_error($con));
 ...
 //$up_fin =$con->query("UPDATE finals set  name='".$_POST["product$i"]."',qty='".$_POST["newdbqty$i"]."'  WHERE id='".$_POST["dbid$i"]."'") or die(mysqli_error($con));
		 ...
		 $update_basket =$con->query("UPDATE  basket SET `status`= '2' ,discount='$discoun',opening_stocks='".$_POST["dbqty$i"]."',closing_stocks='".$_POST["newdbqty$i"]."',casheir='".$_SESSION['userSession']."',date='$cur_date'  WHERE tab='".$table_num."' and ids='".$command_name."'  and status= '5' ") or die(mysqli_error($con));
		 
	


echo "<script>window.open('rec.php?roll=".$who."&area=".$_GET['area']."','_self')</script>";
    ...
    ?>
           ...
           <tr> 
            ...
            <td>Discount</td><td>&nbsp;&nbsp&nbsp;&nbsp;&nbsp;<input type="text" name="disc"  class="input" onBlur="doCalc(this.form)" 
                  value="0"  >
                       ...
                       <tr> 
            <td>Amount Paid</td><td>&nbsp;&nbsp&nbsp;&nbsp;&nbsp;<input type="text" name="paid_m"  class="input" onBlur="doCalc(this.form)" 
                 readonly autocomplete="off"  value="<?php
				 
				 
				  $a = $con->query("SELECT balance as balance FROM basket where tab='".$table_num."' and ids='".$command_name."'  and status='5' GROUP BY balance") or die(mysqli_error($con));
		...
		while($rows = $a->fetch_assoc()) {
		 ...
		 echo $totdebts+$tot; ?>" >
                       ...
                       <td></td><td><input type="hidden" name="paid"  class="input" onBlur="doCalc(this.form)" 
                 readonly autocomplete="off"  value="<?php echo $totdebts; ?>" >
                  </td></tr>
                  
                  
                  <tr> 
            <td></td><td><input type="hidden" name="owed" readonly class="input" 
                  required="required" style="background:#F00; color:#fff" value="<?PHP $a = $con->query("SELECT balance as balance FROM basket where tab='".$table_num."' and ids='".$command_name."'  and status='5' GROUP BY balance") or die(mysqli_error($con));
			
		while($rows = $a->fetch_assoc()) {
			echo  $tot=round($rows['balance']);
		}
		 ...
		 ?>"  >
                     ...
                     <tr> 
            <td>Balance: </td><td>&nbsp;&nbsp&nbsp;&nbsp;&nbsp;<input type="text" name="owed_mmm" readonly class="input" 
                  required="required" style="background:#F00; color:#fff" value="<?PHP $a = $con->query("SELECT balance as balance FROM basket where tab='".$table_num."' and ids='".$command_name."'  and status='5' GROUP BY balance") or die(mysqli_error($con));
			
		while($rows = $a->fetch_assoc()) {
			  $tot=round($rows['balance']);
		}
		if(empty($tot)){
			echo 0;
		}
		 ...
		 ?>"  >
            ...
            <tr>
                ...
                <input type="submit" name="SubmitUpdate" value="Validate and Save" style="background:#033; font-weight:bold; text-transform:uppercase; color:#fff; padding:10px 10px"/></td>
            </tr>
            </table>
            </form>
