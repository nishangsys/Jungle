...
//////////////////////////////////////////////////////////////////////////////////////////////////////
	...
	?>
     ...
     <div style="width:100%; height:auto; overflow:hidden; padding-bottom:20px; float:right; border:3px dashed#FF0; outline:1px dashed#000; background:url(../img/a.gif); "  >
		...
		$area= $_GET['area'];
		echo $t_num;
 $as =$con->query("SELECT * FROM users,basket where basket.agent=users.id and   basket.status='5' and basket.area='$area' and basket.qty>0 GROUP BY basket.agent  " ) or die(mysqli_error($con));
 while($bs=$as->fetch_array()){
   $na=$bs['full_name'];
 $yid=$bs['agent'];
 ...
 ?>
  ...
  <strong><?php echo $na; ?> Orders</strong> 
</div>
...
<!----order details----------->
 ...
 $query =$con->query("SELECT * FROM basket where   status='5' and area='$area' AND agent='".$yid."' and qty>0 GROUP BY ids,tab order by id DESC" ) or die(mysqli_error($con));
...
<?php } ?>  
  ...
  ****/ ?>
 <!--------TOTAL---->   
   <div class="col-sm-12" style="background:#060; padding:0px 0px; text-align:CENTER; border:1px solid#fff">
   
 
     <span style="background:#060; padding:15px 20px; float:right; font-size:20px; font-stretch:expanded; color:#fff; font-weight:bold ">  
      <?PHP
      	$a = $con->query("SELECT SUM(price*qty) as totbill FROM basket where tab='".$t_num."' and ids='".$c_num."' and area='$section'  and status='5' AND agent='".$yid."'") or die(mysqli_error($con));
 ...
 <div style="height:14px; background:#fff; clear:both"></div>
  <?php
