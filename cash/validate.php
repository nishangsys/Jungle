...
if(isset($_POST['SubmitUpdate'])) {
	...
	$_POST = array_map("ucwords", $_POST);
	 ...
	 $discoun=$_POST['disc'];
		...
		$acv=$con->query("INSERT INTO daily set cust_id='$who',room='',amt='$amt_owed',reason=' Bill',qty='1',price='$totalbill',total='$totalbill',
			owed='$amt_owed',date='$date',month='$month',year='$year',area='0',time='$time',paidto='$paidto',purpose='Bills' ,idds='',discount='$discoun',staffname='$name',rec='$amount_paid'") or die(mysqli_error($con));
 ...
 $up_fin =$con->query("UPDATE supermkt set  name='".$_POST["product$i"]."',qty='".$_POST["newdbqty$i"]."'  WHERE id='".$_POST["dbid$i"]."'") or die(mysqli_error($con));
		 
	
		
		 $update_basket =$con->query("UPDATE  basket SET `status`= '2' ,discount='$discoun'  WHERE tab='$who' and status= '0' ") or die(mysqli_error($con));
		 
		  $update_basket =$con->query("UPDATE  customers SET `status`= '2'   WHERE client_id ='$who' ") or die(mysqli_error($con));
...
echo "<script>
window.open('../content/rec.php?roll=".$who."','_new')</script>";
...
<h1 style="background:#088389; text-align:center; font-size:24px; padding:20px 0px; color:#fff;"> <span style="color:#ff0"> <span style="color:#Fff"><a href="?go_ahead&link=<?php echo $name; ?>&roll=<?php echo $_GET['roll']; ?>" style="color:#Fff; text-decoration:none">Go Back to Sales |</a></span> <?php echo $name; ?> Total Bill: 
    ...
    ?>
           ...
           <tr> 
            ...
            <tr>
                ...
                <input type="submit" name="SubmitUpdate" value="Valid Sales" onClick="return confirm('Are you sure about that?')" /></td>
            </tr>
            </table>
            </form>
