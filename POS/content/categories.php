...
if(isset($_POST['ok'])){	
	...
	$dept=$_POST['name'];
	$gh12=$con->query("DELETE FROM categories WHERE name='$dept' ") or die(mysqli_error($con));
	
	
	$gh=$con->query("INSERT INTO categories set name='$dept' ") or die(mysqli_error($con));
...
if(isset($_GET['delete'])){
	$gh=$con->query("delete from categories where id='".$_GET['delete']."' ") or die(mysqli_error($con));
...
<?php $d=$con->query("SELECT * FROM categories order by name   ") or die(mysqli_error($con));
$i=1;
?>
