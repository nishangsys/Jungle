...
<?php
 $id;
   $dm=$con->query("SELECT * FROM hair_stylists where sector='".$_GET['sector']."'") or die(mysqli_error($con));
while($bum=$dm->fetch_assoc()){
	 $deptss=$bum['name'];
     $id=$bum['id'];
?>
<a href="?plattnow&link=Working on my customer&type=<?php echo $deptss; ?>&name=<?php echo $_GET['name']; ?>&sid=<?php echo $_GET['sid']; ?>&style=<?php echo $_GET['type']; ?>&roll=<?php echo $_GET['yid']; ?>&min=<?php echo $_GET['minprice']; ?>&max=<?php echo $_GET['maxprice']; ?>">
