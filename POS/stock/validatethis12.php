...
include '../includes/dbc.php'; // include the database and server connection file
 $who=$_GET['roll'];	
 	   ...
 	   $eern=mysql_query("UPDATE `smkt_whouse` SET `qty`= '0'   WHERE qty<1		  ") or die(mysql_error());
   
$a	= mysql_query("SELECT SUM(current*date),SUM(current) FROM `d_gross` where  taken!='2' GROUP BY taken ORDER BY id ASC") or die(mysql_error()); // Sel
while($bb=mysql_fetch_assoc($a)){
	 $cost=$bb['SUM(current*date)'];
	 $qty=$bb['SUM(current)'];
}

$query 	= mysql_query("SELECT stock,SUM(current),discribe,sentby,id FROM `d_gross` where  taken!='2' GROUP BY stock  ORDER BY id ASC") or die(mysql_error()); // Select from the table
	...
	for($i=1; $i<=$amt; $i++) {
	 ...
	 $_POST['sentothem$i']=$_POST["qty$i"];
	  ...
	  $paidto=$_SESSION['user_name'];
	   ...
	   $upbn=mysql_query("UPDATE `wholesale` SET `qty`= '".$_POST["lefts$i"]."'   WHERE `id` = '".$_POST["pro_id$i"]."' 		  ") or die(mysql_error());
			
		
		 $update_d_gross =mysql_query  ("UPDATE `d_gross` SET `taken`= '2',units='".$_POST["discribe$i"]."' WHERE `stock` = '".$_POST["product$i"]."' and sentto='$who' and taken!='2'  ") or die(mysql_error());
...
echo "<script>window.open('../stock/srec12.php?br=".$who."','_self')</script>";
    ...
    ?>
            ...
            <tr style="background:#1188aa; color:#fff; height:30px; padding:10px 0px">
                ...
                <td align="center" class="tt">Supplier Name</td>
                                <td align="center" class="tt">Deletes</td>
            ...
            <tr>
                ...
                <td><input type="text" name="product<?php echo $i; ?>" value="<?php echo $row['stock']; ?>" class="input" required style="width:190px" readonly   /></td>
                  <td><input type="text" name="cost<?php echo $i; ?>" value="<?php 
				  $df=mysql_query("SELECT * FROM smkt_whouse where name='".$row['stock']."' ") or die(mysql_error());
				  ...
				  ?>" class="input" style="width:130px" readonly  /></td>
                   ...
                   <td><input type="text" name="qty<?php echo $i; ?>" value="<?php echo $row['SUM(current)']; ?>" class="input" style="width:60px" readonly  /></td>
                     ...
                     <td><input type="text" name="lefts<?php echo $i; ?>" value="<?php 
  $a=mysql_query("SELECT * FROM wholesale where name='".$row['stock']."' ") or die(mysql_error());
				  while($b=mysql_fetch_assoc($a)){
					 echo  $dd=$b['qty']+$row['SUM(current)'];
				  }
                 ...
                 <td><a href="deleteit.php?roll=<?php echo $row['id']; ?>&sector=<?php echo $_GET['supp']; ?>" style="color:#F00" onClick="return confirm('Are you sure you wish to delete <?php echo $row['stock']; ?>') ">Delete</a></td>
                  <td><input type="hidden" name="pro_id<?php echo $i; ?>" value="<?php 
			   $s=mysql_query("SELECT *  from wholesale where name='".$row['stock']."' 
			    ") or die(mysql_error());
