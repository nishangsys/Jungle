	...
	for($i=1; $i<=$amt; $i++) {
	 ...
	 $_POST['sentothem$i']=$_POST["qty$i"];
	  ...
	  $paidto=$_SESSION['user_name'];
	   ...
	   $upbn=mysql_query("UPDATE `smkt_whouse` SET `qty`= '".$_POST["lefts$i"]."'   WHERE `id` = '".$_POST["pro_id$i"]."' 		  ") or die(mysql_error());
	   ...
	   $upbn=mysql_query("UPDATE `supermkt` SET `qty`= '".$_POST["lefts$i"]."'   WHERE `id` = '".$_POST["pro_id$i"]."' 		  ") or die(mysql_error());
...
echo "<script>window.open('../stock/srec.php?br=".$who."','_self')</script>";
...
<h1 style="background:#088389; text-align:center; font-size:24px; padding:20px 0px; color:#fff;"><a href="receiving.php?receiving=Admin&branch=BUEA&supp=general" style="color:#FF0; text-decoration:none">&infin; UP BY ONE LEVEL</a> Stock Validation for <span style="color:#ff0">  <?php echo $who; ?>  <br><span style="color:#fff">Qty Supplied: <span style="color:#FF0"><?php echo ($qty); ?> </span>Worth of Distributed Product is </span> <?php echo number_format($cost); ?> Frs</span></h1>
  ...
  $a=mysql_query("SELECT * FROM smkt_whouse where name='".$row['stock']."' ") or die(mysql_error());
