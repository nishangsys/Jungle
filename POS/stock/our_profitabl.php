...
?>
 ...
 <thead>
   ...
   <?php
   $year=date('Y');
    $d=$con->query("select SUM(qty),SUM((price-cp)*qty),product from basket where year='$year' GROUP BY product order by SUM((price-cp)*qty) DESC") or die(mysqli_error($con));
$i=1;
?>
