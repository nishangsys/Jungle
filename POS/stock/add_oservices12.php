...
<?php  include 'search/index.php'; ?>
		...
		$cate=$_GET['cate'];
		if(empty($cate)){
		$d=$con->query("SELECT * FROM finals where qty>0 order by name ") or die(mysqli_error($con));
		}
		else {
			$d=$con->query("SELECT * FROM finals where disc='$cate' and qty>0 order by name ") or die(mysqli_error($con));
		}
$i=1;
?>
 <thead>
