...
?>
 ...
 <thead>
  ...
  <?PHP
								...
								$ab= mysql_query("select SUM(qty),SUM(cost*qty),SUM(sp*qty) from finals where branch like '%$br%'" ) or die (mysql_error());
								$num=1;
								while ($r= mysql_fetch_array ($ab) ){
							?>                                    
   
   <?php $d=$con->query("SELECT * FROM finals where branch like '%$br%' order by id DESC  ") or die(mysqli_error($con));
$i=1;
?>
 ...
 <td><?php  $bc=$df['barcode'];
  $id=$df['id'];
 if(empty($bc)){
	 echo "<a href='../main/barcode/html/BCGcode39.php?id=$id' target='new' style='color:#f00; font-weight:bold' >Print Barcode</a>";
 }
 else {
	 echo $bc;
 }
