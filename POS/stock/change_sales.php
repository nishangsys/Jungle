...
$dept=$_GET['dept'];
	...
	//////////////////update basket	 	
	echo $un1=$con->query ("UPDATE 	basket set qty='$qty'  where id='$adds' ") ;
	//////////////////update basket	
	$d1=$con->query("DELETE FROM 	basket WHERE qty<1") or die(mysqli_error($con)); 
	
	  echo '<meta http-equiv="Refresh" stock="0; url=change_sales.php?tabs='.$_GET['tab'].'">';
  ...
  <div class="col-sm-12">
        ...
        <?php $d=$con->query("SELECT SUM(qty),SUM(total),price,product,category,ids,id,price,qty,total,agent,tab FROM basket where tab='".$_GET['tabs']."' and status='0' and qty>0 GROUP BY category,product order by product ") or die(mysqli_error($con));
$i=1;
?>
 <thead>
                                        <tr>
