...
while($ac=$dfm->fetch_assoc())
	...
	$dfm=$con->query("SELECT * from supermkt WHERE name='$prod' or barcode='$prod' LIMIT 1 ") or die(mysqli_error($con));
while($ac=$dfm->fetch_assoc())
		{
			
			
		echo	 $dfm2=$con->query("SELECT * from basket WHERE product='$prod' and tab='SUP'  and status!='2' ") or die(mysqli_error($con));
	 echo $coum=mysql_num_rows($dfm2);exit;
while($acm=$dfm2->fetch_assoc())
		{
...
while($ac=$dfm->fetch_assoc())
	...
	total='$total',qty='$nqty',cp='$sp',status='',tab='$tab',ids='$id',opening_stocks='$aviail',closing_stocks='$newqty',area='$yarea',profit='$profit',time='$time',
...
while($ac=$dfm->fetch_assoc())
      ...
      <tr>
        ...
        <td><a href="index.php?del=<?php echo $ac['id']; ?>&area=<?php echo $_GET['area']; ?>&roll=<?php echo $_GET['roll']; ?>"><img src="delete.png"></a>
      </tr>
     <?php } ?>
...
<?PHP
 ...
 $update_basket =$con->query("UPDATE  basket SET `status`= '2' ,discount='$discoun',opening_stocks='".$_POST["dbqty$i"]."',closing_stocks='".$_POST["newdbqty$i"]."'  WHERE area='".$yarea."' and status= '0' and tab='".$_GET['roll']."' ") or die(mysqli_error($con));
...
echo "<script>window.open('../Cash/rec_sup.php?area=".$yarea."&br=".$_GET['name']."&change=".$change."&roll=".$_GET['roll']."','_self')</script>";
