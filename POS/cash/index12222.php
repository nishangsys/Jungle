...
$date=date('d-m-Y');
	...
	if(isset($_POST['txtCountry'])){
	 ...
	 $dfm=$con->query("SELECT * from basket WHERE product='$prod' and tab='SUP'  and status!='2' ") or die(mysqli_error($con));
	 $cou=mysqli_num_rows($dfm);
while($ac=$dfm->fetch_assoc())
		{
	////count & check if product exists with sta not 2
	if($cou>1){
		$nqty=1;
	}
...
while($ac=$dfm->fetch_assoc())
		{
			...
			$check=$con->query("DELETE FROM basket WHERE product='$thepro'  AND area='$yyarea' AND status='0'  ") or die(mysqli_error($con));
