...
<div class="container-fluid">
 ...
 <form class="form-inline" role="form" method="post" action="">
  ...
  |
       ...
       <label for="email">
        &nbsp;&nbsp; <a href="../../logout.php" style="color:#fF0; text-decoration:blink text-align:center; font-size:14px; font-weight:bold;"> LOG OUT</a>
	...
	echo $prod=mysql_real_escape_string($_POST['txtCountry']);
	 $sp=$_POST['sp'];
	 
	 ////get name and barcode first
	   $dfm=$con->query("SELECT * from supermkt WHERE name='$prod' or barcode='$prod' ") or die(mysqli_error($con));
while($ac=$dfm->fetch_assoc())
		{
			echo $the_product=$ac['name'];
		}
	 
	 ////CHECK IF A unit have been sold then add +1
	 
	 $dfm=$con->query("SELECT * from basket WHERE product='$the_product'  and tab='SUP'  and status!='2' ") or die(mysqli_error($con));
while($ac=$dfm->fetch_assoc())
		{
	 	$already_sold=$ac['qty'];
		}
	...
	/////if exists add one to current qty
	else {
		echo $nqty=$already_sold+1;
			}
		
		/////check product in stock>>>fin
	 
	$dfm=$con->query("SELECT * from supermkt WHERE name='$the_product'  LIMIT 1 ") or die(mysqli_error($con));
while($ac=$dfm->fetch_assoc())
		{
 ...
 if($sold>$avqty){
  ...
  $dfm=$con->query("SELECT * from supermkt WHERE id='$id' ") or die(mysqli_error($con));
while($ac=$dfm->fetch_assoc())
		{
			...
			$check=$con->query("DELETE FROM basket WHERE product='$thepro'  AND area='$yarea' AND status='0'  ") or die(mysqli_error($con));
...
while($ac=$dfm->fetch_assoc())
      ...
      <tr>
        ...
        <td><a href="index.php?del=<?php echo $ac['id']; ?>&area=<?php echo $_GET['area']; ?>&roll=<?php echo $_GET['roll']; ?>" style="color:#f00; font-weight:bold">Delete</a></td>
        
        <td><a href="index.php?area=<?php echo $_GET['area']; ?>&roll=<?php echo $_GET['roll']; ?>&qty=<?php echo $ac['qty']; ?>&sub=<?php echo $ac['id']; ?>" style="color:#f00; font-weight:bold">Reduce</a></td>
        
         <td><a href="index.php?area=<?php echo $_GET['area']; ?>&roll=<?php echo $_GET['roll']; ?>&qty=<?php echo $ac['qty']; ?>&add=<?php echo $ac['id']; ?>" style="color:#009; font-weight:bold">Add</a></td>
      </tr>
     <?php } ?>
...
<?PHP
 ...
 $up_fin12 =$con->query("UPDATE supermkt set  name='".$_POST["product$i"]."',qty='".$_POST["newdbqty$i"]."'  WHERE id='".$_POST["dbid$i"]."'") or die(mysqli_error($con));
...
echo "<script>window.open('../Cash/rec_sup.php?area=".$yyarea."&br=".$_GET['name']."&change=".$change."&roll=".$_GET['roll']."','_self')</script>";
