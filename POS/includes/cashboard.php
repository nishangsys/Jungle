...
$user=$_SESSION['user_name'];
 if($level=='20' or $level=='5'){
	 ...
	 $d=$con->query("SELECT * FROM users where id='".$_SESSION['id']."'   ") or die(mysqli_error($con));
$i=1;
while($bu=$d->fetch_assoc()){
	$br=$bu['country'];
}

 $d=$con->query("SELECT * FROM branches where name='".$br."'   ") or die(mysqli_error($con));
$i=1;
while($bu=$d->fetch_assoc()){
	$brid=$bu['id'];
}
  ...
  <div class="container-fluid">
    ...
    <div class="collapse navbar-collapse" id="myNavbar">
      <ul class="nav navbar-nav">
         ...
         <li>   <a href=javascript:popcontact('../content/add_oservices.php?roll=1&name=<?php echo $brid; ?>') style="color:#000">   Sell Now</a></li>
      ...
      <ul class="nav navbar-nav navbar-right">
        <li><a href="../logout.php" style="color:#00F; font-weight:bold"> <i class="icon-user " style="color:#00F"></i> <?php echo $user; ?> of <?php echo $br; ?> | <i class="icon-off " style="color:#F00"></i>   Log Out</a></li>
      </ul>
    </div>
  </div>
</nav>
