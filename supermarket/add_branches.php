...
if(isset($_POST['ok'])){	
	...
	$dept=mysql_real_escape_string($_POST['name']);
	$gh12=$con->query("DELETE FROM branches WHERE name='$dept' ") or die(mysqli_error($con));
	
	
	$gh=$con->query("INSERT INTO branches set name='$dept' ") or die(mysqli_error($con));
...
if(isset($_GET['delete'])){
	$gh=$con->query("delete from branches where id='".$_GET['delete']."' ") or die(mysqli_error($con));
		...
		?>
           <td><?php  echo $i++; ?></td>
                                            <td><?php echo $bu['name']; ?></td>  
                                              <td><a href="?add_branches&link=Add branches&delete=<?php echo $bu['id']; ?>&dept=<?php echo $_GET['dept'];?>" style="font-weight:bold; color:#033"><button class="btn btn-danger"><i class="icon-remove icon-white"></i> Delete</button></a></td>
