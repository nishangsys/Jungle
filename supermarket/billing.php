...
$dept=$_GET['br'];
	...
	?>
        ...
        <a href=javascript:popcontact('validate.php?roll=1&name=<?php echo $dept; ?>') style="color:#fff; text-decoration:blink text-align:center; font-weight:bold;">   
        <div class="col-sm-12" style="padding:10PX 0PX; background:#300;border:1PX solid#fff ;text-align:center; font-weight:bold;">   
   Validate
        </div></A>
 ...
 <!--------TOTAL---->   
   <div class="col-sm-12" style="background:#003; padding:0px 0px; text-align:CENTER; border:1px solid#fff">   <span style="background:#090; padding:15px 45px; float:left; font-size:18px; color:#fff; font-weight:bold ">  TOTAL BILL</span>
     ...
     <span style="background:#FF3; padding:15px 20px; float:right; font-size:18px; color:#000; font-weight:bold ">  
      <?PHP
      	$a = $con->query("SELECT SUM(price*qty) as totbill FROM basket where tab='".$_GET['tabs']."' and status='0' and froms='$dept' GROUP BY tab") or die(mysqli_error($con));
       ...
       <!-----TOTAL--------->
    	...
    	<div class="chat_data">
            ...
            <?PHP
			$a = $con->query("SELECT SUM(qty),SUM(total),price,product,category,ids,id,price,qty,total,agent FROM basket where tab='".$_GET['tabs']."' and status='0' and qty>0 and froms='$dept' GROUP BY category,product order by product ") or die(mysqli_error($con));
			$i=1;
			?>
