...
?>
 ...
 <thead>
   ...
   <?php
   $year=date('Y');
    $d=$con->query("select SUM(qty),SUM((price-cp)*qty),product,price,cp from basket where year='$year' GROUP BY product order by SUM(qty) DESC") or die(mysqli_error($con));
$i=1;
?>
      ...
      <tr>
        ...
        <td><?php echo $df['SUM(qty)']; ?></td>
        
        <td><?php echo $name=$df['cp']; ?></td>
        <td><?php echo $df['price']; ?></td>
         <td><?php echo number_format($df['SUM((price-cp)*qty)']); ?></td>
