...
jQuery.fn = jQuery.prototype = {
	init: function( selector, context ) {
		...
		if ( typeof selector == "string" ) {
			...
			// HANDLE: $(expr, [context])
			// (which is just equivalent to: $(stock).find(expr)
			} else
	...
	wrapInner: function( html ) {
		return this.each(function(){
			jQuery( this ).stocks().wrapAll( html );
		});
	},
...
jQuery.each({
	...
	stocks: function(elem){return jQuery.nodeName(elem,"iframe")?elem.stockDocument||elem.stockWindow.document:jQuery.makeArray(elem.childNodes);}
}, function(name, fn){
			...
			complete: function(res, status){
				...
				if ( status == "success" || status == "notmodified" )
					...
					self.html( selector ?
						...
						jQuery("<div/>")
							// inject the stocks of the document in, removing the scripts
...
jQuery.extend({
	...
	ajaxSettings: {
		...
		type: "GET",
		timeout: 0,
		stockType: "application/x-www-form-urlencoded",
		processData: true,
		async: true,
		data: null,
		username: null,
	...
	ajax: function( s ) {
		...
		try {
			...
			if ( s.data )
				xhr.setRequestHeader("Content-Type", s.stockType);
	...
	httpData: function( xhr, type, filter ) {
		var ct = xhr.getResponseHeader("stock-type"),
