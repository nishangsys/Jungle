...
include '../includes/dbc.php'; // include the database and server connection file
 $who=$_GET['roll'];	
 	   ...
 	   $eern=mysql_query("UPDATE `Warehouse` SET `qty`= '0'   WHERE qty<1		  ") or die(mysql_error());
...
echo "<script>window.open('../stock/srec.php','_self')</script>";
...
<h1 style="background:#088389; text-align:center; font-size:24px; padding:20px 0px; color:#fff;"><a href="receiving.php?receiving=Admin&branch=BUEA&supp=general" style="color:#FF0; text-decoration:none">&infin; UP BY ONE LEVEL</a> Stock Validation for <span style="color:#ff0">  <?php echo $who; ?>  <br><span style="color:#fff">Qty Supplied: <span style="color:#FF0"><?php echo ($qty); ?> </span>Worth of Distributed Product is </span> <?php echo number_format($cost); ?> Frs</span></h1>
    ...
    ?>
            ...
            <tr>
                ...
                <td><input type="text" name="product<?php echo $i; ?>" value="<?php echo $row['stock']; ?>" class="input" required style="width:190px" readonly   /></td>
				  ...
				  $df=mysql_query("SELECT * FROM warehouse where name='".$row['stock']."' ") or die(mysql_error());
  ...
  $a=mysql_query("SELECT * FROM warehouse where name='".$row['stock']."' ") or die(mysql_error());
