...
$o=$con->query("UPDATE  smkt_whouse SET qty='$tqty', barcode='$barcode'  WHERE id='$id'") or die(mysqli_error($con));
...
if(isset($_GET['delete'])){
  ...
  /////////////for updates
  $doU=$con->query("SELECT * FROM smkt_whouse WHERE id='".$_GET['update']."'") or die(mysqli_error($con));
  ...
  if(isset($_POST['Update'])){
	  ...
	  $Dis=$_POST['disc'];
	   ...
	   $qtty=$_POST['qtty'];
	 $id=$_GET['update'];
	  $dfGu=$con->query("UPDATE smkt_whouse SET name='$shape',disc='$Dis',cost='$C',sp='$S',qty='$qtty' where id='$id'") or die(mysqli_error($con));
	  
	  
	 $dfGu=$con->query("UPDATE supermkt SET name='$shape',disc='$Dis',cost='$C',sp='$S' where name='$shape' and disc='$Dis'") or die(mysqli_error($con));
...
$qty=$_POST['qty'];
$sp=$_POST['sp'];
//$df=$con->query("DELETE FROM smkt_whouse where name='$shape' and disc='$disc' and branch='$branch'") or die(mysqli_error($con));
$o=$con->query("SELECT * FROM smkt_whouse WHERE name='$shape' and disc='$disc' and branch='$branch'") or die(mysqli_error($con));
while($cc=$o->fetch_assoc()){
	$av=$cc['qty'];
	$nqty=$av+$qty;
}
if(mysqli_num_rows($o)>0){
	$dfGu=$con->query("UPDATE smkt_whouse SET qty='$nqty',cost='$cost',name='$shape' WHERE name='$shape' AND disc='$disc'") or die(mysqli_error($con));
	   $message= "<div class='alert alert-success'>Item Successfully Updated. Thank You</div>";
}
else {
...
$do=$con->query("INSERT INTO smkt_whouse SET name='$shape',disc='$disc',branch='$branch',qty='0',cost='$cost',sp='$sp'  ") or die(mysqli_error($con));
...
if(isset($_GET['delete'])){
	 $id=$_GET['delete'];
	  $dfG=$con->query("DELETE FROM smkt_whouse where id='$id'") or die(mysqli_error($con));
	   $message= "<div class='alert alert-danger'>Item Successfully Deleted. Thank You</div>";
	    echo '<meta http-equiv="Refresh" stock="0; url=?add_goods.php ">';

  }
  
  /////////////for updates
  $doU=$con->query("SELECT * FROM smkt_whouse WHERE id='".$_GET['add_good']."'") or die(mysqli_error($con));
  while($nam=$doU->fetch_assoc()){
	 $sha=$nam['name'];
	  $dis=$nam['disc'];
	  $cp=$nam['cost'];
  ...
  if(isset($_POST['Update'])){
	 ...
	 $id=$_GET['update'];
	 $qtty=$_POST['qtty'];
	  $dfGu12=$con->query("UPDATE smkt_whouse SET name='$shape',disc='$Dis',cost='$C',sp='$S',qty='$qtty' where id='$idd'") or die(mysqli_error($con));
	  
	  $d899=$con->query("UPDATE supermkt SET name='$shape' where name='$oldname'") or die(mysqli_error($con));
	   $message= "<div class='alert alert-success'>Item Successfully Updated. Thank You</div>";
	   
		 echo '<meta http-equiv="Refresh" stock="0; url=?recordin_goods ">';
  ...
  <div class="form-group">
   ...
   <div class="form-group">
    <label class="control-label col-sm-2" for="pwd"> </label>
    <div class="col-sm-10"> 
      <input type="text"  class="form-control" id="pwd" value="<?php echo $dis; ?>" name="disc"  placeholder="Units of Measureemts:" style="background:#FF9; border:2px solid#f00" >
    </div>
  </div>
 
 <input type="hidden"  class="form-control" id="pwd" value="<?php echo $sha; ?>" name="oldname"   >

   <div class="form-group">
    <label class="control-label col-sm-2" for="pwd"> </label>
    <div class="col-sm-10"> 
      <input type="text"  class="form-control" id="pwd" value="<?php echo $cp; ?>" name="cost"  placeholder="Cost Price:" style="width:120px;background:#FF9; border:2px solid#f00"  >
    </div>
  </div>
    ...
    <label class="control-label col-sm-2" for="pwd"></label>
    <div class="col-sm-10"> 
      <input type="text"  class="form-control" id="pwd" value="<?php echo $sp; ?>" name="sp" placeholder="Selling Price:" style="width:120px;background:#FF9; border:2px solid#f00"  >
    </div>
  </div>  
...
?>
 ...
 <td><?php echo $df['barcode']; ?></td>
         <td>
          ...
          <a href="../stock/UPDATE/index.php" target="new">
<button class="btn btn-success" ><i class="icon-barcode icon-white"></i> New Barcode</button>
