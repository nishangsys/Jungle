...
$gh=$con->query("INSERT INTO smkt_whouse SET qty='$qty',name='$product',month='$month',date='$date',year='$year',cp='".$_GET['cost']."',sp='".$_GET['sp']."' ") or die(mysqli_error($con));
...
?>
 ...
 <thead>
  ...
  <?PHP
								...
								$ab= mysql_query("select SUM(qty),SUM(cost*qty),SUM(sp*qty) from smkt_whouse" ) or die (mysql_error());
								$num=1;
								while ($r= mysql_fetch_array ($ab) ){
							?>                                    
   
   <?php $d=$con->query("SELECT * FROM smkt_whouse order by id DESC  ") or die(mysqli_error($con));
$i=1;
?>
