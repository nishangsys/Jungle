...
$query 	= mysql_query("SELECT stock,SUM(current),discribe,sentby,id FROM `truck` where  taken!='2' GROUP BY stock  ORDER BY id ASC") or die(mysql_error()); // Select from the table
	...
	for($i=1; $i<=$amt; $i++) {
	 ...
	 $_POST['sentothem$i']=$_POST["qty$i"];
	  ...
	  $paidto=$_SESSION['user_name'];
	   ...
	   $upbn=mysql_query("UPDATE `Warehouse` SET `qty`= '".$_POST["lefts$i"]."'   WHERE `name` = '".$_POST["product$i"]."' 		  ") or die(mysql_error());
	   
	   
	   $upbn=mysql_query("UPDATE `supermkt` SET `qty`= '".$_POST["newstock$i"]."' , `barcode`= '".$_POST["barcode$i"]."'   WHERE `name` = '".$_POST["product$i"]."' 		  ") or die(mysql_error());
...
echo "<script>window.open('../stock/ssrec.php','_self')</script>";
...
<h1 style="background:#088389; text-align:center; font-size:24px; padding:20px 0px; color:#fff;"><a href="supplies.php?roll=1&name=customer" style="color:#FF0; text-decoration:none">&infin; UP BY ONE LEVEL</a> Stock Validation for <span style="color:#ff0">  <?php echo $who; ?>  <br><span style="color:#fff">Qty Supplied: :<span style="color:#FF0"><?php echo ($qty); ?> </span>..Worth of Distributed Product is </span> <?php echo number_format($cost); ?> Frs</span></h1>
    ...
    ?>
            ...
            <tr>
                ...
                <td><input type="text" name="product<?php echo $i; ?>" value="<?php echo $row['stock']; ?>" class="input" required style="width:190px" readonly   /></td>
                 ...
                 <td><a href="deleteit12.php?roll=<?php echo $row['id']; ?>&sector=<?php echo $_GET['supp']; ?>" style="color:#F00" onClick="return confirm('Are you sure you wish to delete <?php echo $row['stock']; ?>') ">Delete</a></td>
  ...
  <td><input type="hidden" name="tc<?php echo $i; ?>" value="<?php echo $cp*$row['SUM(current)'];
        ...
        <td><input type="hidden" name="newstock<?php echo $i; ?>" value="<?php 
		  $s=mysql_query("SELECT *  from supermkt where name='".$row['stock']."' 
			    ") or die(mysql_error());
				while($d=mysql_fetch_assoc($s)){
					echo $pri=$d['qty']+$row['SUM(current)'];
