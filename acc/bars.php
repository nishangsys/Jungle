...
$query 	= mysql_query("SELECT * FROM `barproducts` ORDER BY pro_id ASC"); // Select from the table
...
if(isset($_POST['submit'])) {
	...
	if($amt > 0) {
		...
		for($i=1; $i<=$amt; $i++) {
	  $check="SELECT * from barproducts where product='".$_POST["product$i"]."' AND category='".$_POST["cate$i"]."' AND quatity>1";
		...
		$qry = "INSERT INTO barproducts(product, category, price, quatity,total,date,status,month,year,agent,barcode,serial) VALUES "; // Split the mysql_query
