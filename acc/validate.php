...
$who=$_GET['roll'];	
   $query 	= mysql_query("SELECT * FROM `basket` where tab='$who' and status='1' and printed='2'  ORDER BY id ASC") or die(mysql_error()); // Select from the table
...
$query 	= mysql_query("SELECT * FROM `basket` where tab='$who' and status='1'  ORDER BY id ASC") or die(mysql_error()); // Select from the table
...
if(isset($_POST['SubmitUpdate'])) {
	...
	for($i=1; $i<=$amt; $i++) {
		...
		,`total`= '".$_POST['allptotal$i']."'   WHERE `product` = '".$_POST["product$i"]."' and `category`= '".$_POST["category$i"]."'") or die(mysql_error()); // Run the Mysql update query inscourse_ide for loop
	...
	//historique insertion
		...
		//daily records
			$daily=mysql_query("INSERT INTO daily set cust_id='restau',room='".$_POST["table$i"]."',
			rec='".$_POST["total$i"]."',date='$date',month='$month',year='$year',area='".$_POST["ids$i"]."',time='$time' ") or die(mysql_error());
			
		 $update_basket =mysql_query  ("UPDATE `basket` SET `status`= '2' WHERE tab='$who' and status='1' ") or die(mysql_error());
	           		 $update_tab =mysql_query  ("UPDATE `our_tables` SET `status`= '2' WHERE num='$who' and status='1' ") or die(mysql_error());
...
<h1 style="background:#088389; text-align:center; font-size:24px; padding:20px 0px; color:#fff;">Goods Validation for <span style="color:#ff0"> <?php echo $youno; ?> </span></h1>
