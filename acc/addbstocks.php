...
if(isset($_POST['submit'])) {
	...
	if($amt > 0) {
	  ...
	  $check="SELECT * from products where product='".$_POST["product$i"]."' AND category='".$_POST["cate$i"]."' and serial='".$_POST["serial$i"]."'";
		...
		$expense = "INSERT INTO expense(product, category,total,date,status,month,year,area) VALUES "; // Split the mysql_query
				$daily = "INSERT INTO daily(cust_id, room,date,rec,exp,month,year,area,time) VALUES "; // Split the mysql_query

		$qry = "INSERT INTO products(product, category, price, quatity,total,date,status,month,year,agent,barcode,serial,expense) VALUES "; // Split the mysql_query
...
'$date', '$status',
'$month', '$year','3'), "; // loop the mysql_query values to avoid more server loding time

$daily .= "('bar', 'exp', '$date',rec='purchase','".$_POST["total$i"]."','$month', '$year','2','".$_POST["product$i"]."'), "; // loop the mysql_query values to avoid more server loding time
...
'$date', '$status',
'$month', '$year', NOW(),'".$_POST["barcode$i"]."', '".$_POST["serial$i"]."','".$_POST["total$i"]."'), "; // loop the mysql_query values to avoid more server loding time
		...
		$daily	= substr($daily, 0, strlen($daily)-2);
		$insert12 = mysql_query($daily) or die (mysql_error()); // Execute the mysql_query
	}
...
<div class="as_wrapper">
	<h1 class="h1"><a href="">You are Adding stocks to the Bar/Snacks</a></h1>
        ...
        ?>
            ...
            <tr>
                ...
                <td><select name="cate<?php echo $i; ?>" class="input" required="required" />
                  <?php $amou="SELECT * from barproducts";
        ...
        <option></option>
		 ...
		 ?>      
                ...
                <td><input type="text" name="price<?php echo $i; ?>" class="input" required /></td>
                 ...
                 <td><input type="hidden" name="serial<?php echo $i; ?>" class="input" /></td>
