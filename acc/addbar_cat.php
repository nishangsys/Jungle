<?php
/**
Simple barproducts Create, Read, Update and Delete (CRUD) using php mysql by asif18.com, for more tutorials visit: http://www.asif18.com
for this tutorial visit: http://asif18.com/20/php/barproducts-insert-update-using-php-mysql-delete-barproducts-records-using-checkboxes-in-php/
**/
include '../dbc.php'; // include the database and server connection file
$query 	= mysql_query("SELECT * FROM `barproducts` ORDER BY cat_id ASC"); // Select from the table
...
if(isset($_POST['submit'])) {
	...
	$amt = $_POST['total'];
	if($amt > 0) {
		$qry = "INSERT INTO barproducts(cat) VALUES "; // Split the mysql_query
		for($i=1; $i<=$amt; $i++) {
...
// barproducts delete case using checkboxes
if(isset($_POST['SubmitDelete'])) {
	$_POST = array_map("ucwords", $_POST);
	$amt = $_POST['total']; // Get the total rows
	for($i=1; $i<=$amt; $i++) {
		$delete = mysql_query("DELETE FROM barproducts WHERE cat_id = '".$_POST["delete$i"]."'"); // Run the delete query inscat_ide for loop
	}
...
// barproducts update case
if(isset($_POST['SubmitUpdate'])) {
	$_POST = array_map("ucwords", $_POST);
	$amt = $_POST['total']; // Get the total rows
	for($i=1; $i<=$amt; $i++) {
		$update = mysql_query("UPDATE `barproducts` SET  cat = '".$_POST["cat$i"]."' WHERE `cat_id` = '".$_POST["cat_id$i"]."'"); // Run the Mysql update query inscat_ide for loop
	}
...
<title>NSMS</title>
<meta name="keywords" content="barproducts insert in php, barproducts crud using PHP MySql, barproducts insert update delete using php mysql"/>
<meta name="description" content="barproducts insert update delete CRUD using PHP MySql. A simple way to insert, update and delete barproducts values at a time using PHP MySql"/>
<style>
.as_wrapper{
	margin:0 auto;
	width:800px;
...
<div class="as_wrapper">
	<h1 class="h1"><a href="">YOU ARE ADDING PRODUCTS TO OUR BAR</a></h1>
