...
echo "<script>window.open('login.php','_self')</script>";
    ...
    ?>
	  ...
	  $month=date('m');
	  $YEAR=date('Y');
	  $one="select SUM(rec) as income from daily WHERE  year='$YEAR' GROUP BY year ";
	   $run=mysql_query($one) or die ('could not updated:'.mysql_error());
	   while($r=mysql_fetch_assoc($run)){
		   $income=$r['income'];
	   }
	   $one="select SUM(exp) as expense from daily WHERE  year='$YEAR' GROUP BY year ";
	   $run=mysql_query($one) or die ('could not updated:'.mysql_error());
	   while($r=mysql_fetch_assoc($run)){
		   $expense=$r['expense'];
	   }
	   $one="select SUM(amt) as profit from daily WHERE  year='$YEAR' GROUP BY year ";
	   $run=mysql_query($one) or die ('could not updated:'.mysql_error());
	   while($r=mysql_fetch_assoc($run)){
		   $profit=$r['profit'];
	   }
 ...
 <div class="right">
   ...
   <h1>
YEARLY SECTORIAL FINANCIAL RECORDS FOR <?php echo date('Y'); ?><br /></h1>
