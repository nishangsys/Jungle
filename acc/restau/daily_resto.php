...
<a href="dailyprint.php" target="_blank" >Click here To print Today's Report</a> 
 ...
 $cust="SELECT room,time,SUM(rec) from daily where date='$today' and area='3'  and rec>0 GROUP BY time   order by time DESC ";
	...
	$cust="SELECT room,time,SUM(owed) from daily where date='$today' and area='3' and paidtou='room services'  GROUP BY room   order by id DESC ";
	$run=mysql_query($cust) or die (mysql_error());
	...
	$cu="SELECT SUM(exp)as exp  from daily WHERE  cust_id='bar'  and area='3' and date='$today'  ";
