...
<body>
   ...
   <div class="subcontain">
     <h1>Please Click on PRINT to print Restaurant Balancesheet</h1>
...
if(isset($_POST['seen'])){
	...
	$year=$_POST['year'];
 echo $sele="SELECT SUM(rec),SUM(exp),SUM(amt),date,id FROM daily where month='$month' or month='$month1'  and year='$year' and area='15'  GROUP BY date";
...
<table>
 ...
 <tr style="background:#1188AA; padding:10PX 0PX; height:30px; color:#fff; text-align:center; font-weight:bold">
   ...
   <td>Total</td>
   <td></td>
   <td><?php  $sele="SELECT SUM(rec),SUM(amt) FROM daily where month='$month' or month='$month1' and year='$year'  and area='15' GROUP BY month";
	$runs=mysql_query($sele) or die (mysql_error());
   ...
   <td><?php  $sele="SELECT SUM(exp) FROM daily where month='$month' or month='$month1' and year='$year'  and area='15' GROUP BY month";
	$runs=mysql_query($sele) or die (mysql_error());
	 while($rows=mysql_fetch_assoc($runs)){
