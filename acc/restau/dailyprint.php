...
<div class="eachrec" style="border-bottom:none;"  >
    ...
    <h1>Restaurant Daily Records</h1>
  <?php
	
	 $today=date('d-m-Y');
	
	
	$cust="SELECT room,time,SUM(rec) from daily where date='$today' and area='3' and rec>0 GROUP BY time   order by time DESC ";
	...
	$cust="SELECT SUM(rec) from daily WHERE  daily.date='$today' and area='3'  ";
	$run=mysql_query($cust) or die (mysql_error());
	...
	$cust="SELECT room,time,SUM(owed) from daily where date='$today' and area='3' and owed>0   GROUP BY room   order by id DESC ";
	$run=mysql_query($cust) or die (mysql_error());
	...
	$cust1=mysql_query("SELECT SUM(owed) from daily where date='$today' and area='3'  GROUP BY date   ") or die(mysql_error());	
    ...
    <td style="color:#000; text-align:center"><?php
	 ...
	 $today=date('d-m-Y');
	$custi="SELECT SUM(exp)as expenses  from daily WHERE  daily.date='$today' and area='3' order by daily.id DESC ";
	$runs=mysql_query($custi) or die (mysql_error());
	while($row=mysql_fetch_assoc($runs)){
		$total1=$row['expenses'];
		echo number_format($total1)."&nbsp; Frs";;
	}
	?></td>
...
echo number_format($totinc-$total1);
	?></td></tr>
