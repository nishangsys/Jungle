...
$who=$_GET['roll'];	
    $query 	= mysql_query("SELECT * FROM `basket` where tab='$who' and printed='0' and status='1'  ORDER BY id ASC") or die(mysql_error()); // Select from the table
...
if(isset($_POST['SubmitUpdate'])) {
	...
	//daily records
			...
			rec='".$_POST["total$i"]."',date='$date',month='$month',year='$year',area='15',time='$time',paidto='$paidto',purpose='bar/restau' ,idds='".$_POST["ids$i"]."'") or die(mysql_error());
	...
	//historique insertion
		 ...
		 $update_basket =mysql_query  ("UPDATE `basket` SET `status`= '2' WHERE tab='$who' and status='1' AND area='15'  ") or die(mysql_error());
		 
		  $update_basket12 =mysql_query  ("UPDATE `all_basket` SET `status`= '2' WHERE tab='$who' and status='1' AND area='15'  ") or die(mysql_error());
