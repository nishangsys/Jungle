...
<script type="text/javascript" src="../search/js/script.js"></script>
      ...
      <tr>
      <td> Product:</td><td>
        <input type="text" class="form-control" placeholder=" Your Name" name="names" id="country_id" onkeyup="autocomplet()" autocomplete="off" style="width:140px"  >                    <ul id="country_list_id"></ul>
...
$d=$con->query("INSERT INTO daily set reason='$sector drinks',date='$date',month='$m',year='$y',qty='$qty',exp='$cost',price='$cost',total='$cost',area='$area',paidto='$who',purpose='$names'") or die(mysqli_error($con));
...
$a=1;
   ...
   <?php while($df=$d->fetch_assoc()){ ?>
    <td><?php echo $a++; ?></td><td>
   <?php echo $df['purpose']; ?></td><td><?php echo $df['qty']; ?></td><td><?php echo $df['total']; ?></td><td><?php echo $_GET['sector']; ?></td><td><a href="?saving&date=<?php echo $df['date']; ?>&area=<?php echo $df['area']; ?>&sector=<?php echo $_GET['sector']; ?>&m=<?php echo $df['month']; ?>&y=<?php echo $df['year']; ?>&del=<?php echo $df['id']; ?>">Delete</a></td></tr>
    <?php } ?>
