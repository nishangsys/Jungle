...
<div class="eachrec" style="border-bottom:none;"  >
    ...
    <div class="oth">
    <h1><?php echo $clients; ?> Bar Balancesheet</h1>
    <h2><?php echo $AD ?></h2>
 ...
 if(isset($_POST['print'])){
	$month=$_POST['month'];
	$year=$_POST['finy'];
 $sele="SELECT SUM(rec),SUM(exp),date,id FROM daily where month='$month' and year='$year' and area='2' GROUP BY date";
...
<tr style="background:#fff; padding:10PX 0PX; height:30px; color:#000; text-align:center; font-weight:bold">
   ...
   <td>Total</td>
   <td></td>
   <td><?php  $sele="SELECT SUM(rec) FROM daily where month='$month' and year='$year'  and area='2' GROUP BY month";
	$runs=mysql_query($sele) or die (mysql_error());
   ...
   <td><?php  $sele="SELECT SUM(exp) FROM daily where month='$month' and year='$year'  and area='2' GROUP BY month";
	$runs=mysql_query($sele) or die (mysql_error());
	 while($rows=mysql_fetch_assoc($runs)){
		echo  number_format($seen=$rows['SUM(exp)']);
	 }?> Frs</td>
   <td style="border-bottom:1px solid#000"><?php echo number_format($ROW-$seen); ?> Frs</td>
