...
$query 	= mysql_query("SELECT * FROM `foods` ORDER BY pro_id ASC"); // Select from the table
...
if(isset($_POST['submit'])) {
	...
	if($amt > 0) {
		...
		for($i=1; $i<=$amt; $i++) {
	  $check="SELECT * from foods where product='".$_POST["product$i"]."' AND category='".$_POST["cate$i"]."' AND quatity>1";
		...
		$qry = "INSERT INTO foods(product, category, price, quatity,total,date,status,month,year,agent,barcode,serial) VALUES "; // Split the mysql_query
...
'$month', '$year'), "; // loop the mysql_query values to avoid more server loding time
		...
		$daily .= "('bar', 'exp', '$date',rec='mm','".$_POST["total$i"]."','$month', '$year','2','$time'), "; // loop the mysql_query values to avoid more server loding time
        ...
        ?>
            ...
            <tr>
                ...
                <td><select name="cate<?php echo $i; ?>" class="input" required="required" />
                  <?php $amou="SELECT * from restauproducts";
        ...
        <option></option>
		 ...
		 ?>      
                ...
                <td><input type="text" name="price<?php echo $i; ?>" class="input" required /></td>
                 ...
                 <td><input type="hidden" name="qty"  /></td>
