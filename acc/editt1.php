...
?>
    ...
    <table>
          ...
          <tr><td></td><td><input type="hidden" name="id" value="<?php echo  $row['id']; ?>" style="width:300px" readonly="readonly" /></td></tr>          
                 ...
                 <tr><td> Expense</td><td><input type="text" name="old" readonly="readonly"value="<?php echo  $row['exp']; ?>"    /></td></tr>
...
<?php if(isset($_POST['saves'])){
		...
		$date=$_POST['date'];
		$old=$_POST['old'];	
		$update_finances=mysql_query("UPDATE daily set exp='$new' where id='".$id."'") or die(mysql_error());
		//daily records
		//$update_client=mysql_query("UPDATE ourclients set bal='".$exp."',paid='$totpaid' where yourid='".$id."'") or die(mysql_error());
$histo=mysql_query("
INSERT INTO reports set date='$date',howmuch='$old',paidin='$new',reason='edited expense',name='$user' ") or die(mysql_error());	
		
	
			
			echo "<script>alert('SUCCESS')</script>";
						echo '<meta http-equiv="Refresh" content="0; url=?mine1">';
