<?php
/**
Simple restauproducts Create, Read, Update and Delete (CRUD) using php mysql by asif18.com, for more tutorials visit: http://www.asif18.com
for this tutorial visit: http://asif18.com/20/php/restauproducts-insert-update-using-php-mysql-delete-restauproducts-records-using-checkboxes-in-php/
**/
include '../dbc.php'; // include the database and server connection file
$query 	= mysql_query("SELECT * FROM `restauproducts` ORDER BY cat_id ASC"); // Select from the table
...
if(isset($_POST['submit'])) {
	...
	$amt = $_POST['total'];
	if($amt > 0) {
		$qry = "INSERT INTO restauproducts(cat) VALUES "; // Split the mysql_query
		for($i=1; $i<=$amt; $i++) {
			$qry .= "( '".$_POST["cat$i"]."'), "; // loop the mysql_query values to avocat_id more server loding time
		}
...
// restauproducts delete case using checkboxes
if(isset($_POST['SubmitDelete'])) {
	$_POST = array_map("ucwords", $_POST);
	$amt = $_POST['total']; // Get the total rows
	for($i=1; $i<=$amt; $i++) {
		$delete = mysql_query("DELETE FROM restauproducts WHERE cat_id = '".$_POST["delete$i"]."'"); // Run the delete query inscat_ide for loop
	}
...
// restauproducts update case
if(isset($_POST['SubmitUpdate'])) {
	$_POST = array_map("ucwords", $_POST);
	$amt = $_POST['total']; // Get the total rows
	for($i=1; $i<=$amt; $i++) {
		$update = mysql_query("UPDATE `restauproducts` SET  cat = '".$_POST["cat$i"]."' WHERE `cat_id` = '".$_POST["cat_id$i"]."'"); // Run the Mysql update query inscat_ide for loop
	}
...
<title>NSMS</title>
<meta name="keywords" content="restauproducts insert in php, restauproducts crud using PHP MySql, restauproducts insert update delete using php mysql"/>
<meta name="description" content="restauproducts insert update delete CRUD using PHP MySql. A simple way to insert, update and delete restauproducts values at a time using PHP MySql"/>
<style>
.as_wrapper{
	margin:0 auto;
	width:900px;
...
.input {
	border:#CCC solcat_id 1px;
	padding:5px;
}

.input:focus {
	border:#999 solcat_id 1px;
...
<body>
<div class="as_wrapper" style="width:800PX">
	<h1 class="h1"><a href="">YOU ARE ADDING FOOD VARITIES  TO OUR RESTAURANT</a></h1>
    ...
    ?>
        ...
        ?>
            ...
            <tr class="<?php if($i%2 == 0) { echo "odd"; } else{ echo "even"; } ?>">
                <td><input type="checkbox" name="delete<?php echo $i; ?>" value="<?php echo $row['cat_id']; ?>" /></td>
                <td><?php echo $i; ?></td>
        ...
        elseif(isset($_GET['update'])) {
            ...
            ?>
            <tr>
                <td><?php echo $i; ?><input type="hidden" name="cat_id<?php echo $i; ?>" value="<?php echo $row['cat_id']; ?>" /></td>
