...
?>                     <tr>
   ...
   <td><?php   $n=$con->query("SELECT SUM(exp) as exp FROM  daily where  purpose='".$df['product']."' and area='".$sector."' and year='$year' ") or die(mysqli_error($con));
  while($mf=$n->fetch_assoc()){
	$totex=$mf['exp'];
	echo  number_format($totex=$mf['exp']);
  }?></td>
  
    <td><?php   $n=$con->query("SELECT SUM(rec) as exp FROM  daily where  reason='".$df['product']."' and area='".$sector."' and year='$year' ") or die(mysqli_error($con));
  while($mf=$n->fetch_assoc()){
	$totin=$mf['exp'];
  ...
  <?php } ?>
   ...
   $dm=$con->query("SELECT SUM(rec),SUM(exp) FROM daily where year='$year' and area='$sector' GROUP BY year") or die(mysqli_error($con));
   $i=1;
