...
<a href="print_mine.php" target="_blank" >Click here To print Today's Report| <a href="../print_to_pdf/print_pdf.php?me=<?php echo $_SESSION['user_name']; ?>" target="_blank">PDF</a></a> 
	...
	$cust="SELECT room,time,SUM(rec),reason,paidto,id,exp from daily where paidto='$user' and date='$today'  and exp>0  GROUP BY id  order by id DESC ";
    ...
    <td><a href="?editt1=<?php echo $row['id'] ?>">Edit</a></td>
	...
	$cust1=mysql_query("SELECT SUM(exp) from daily where  paidto='$user' and date='$today'     ") or die(mysql_error());	
	while($row2=mysql_fetch_assoc($cust1)){
		echo number_format($ans=$row2['SUM(exp)']);
