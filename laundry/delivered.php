...
$y=date('Y');
	$codes=$code;
		...
		if($old>0){
			echo "<script>alert('Sorry let this client settle his or her debt of $old')</script>";
		}
	 ...
	 //////update service_offered
	    $dfkl=$con->query("UPDATE service_offered set status='4' where code='$codes' ") or die(mysqli_error($con));
  ...
  <form class="form-horizontal" action="" style="margin-top:40px" method="post" >
    ...
    <div class="form-group">
      <div class="col-sm-offset-2 col-sm-10">
        <button type="submit" class="btn btn-primary" name="OK" onclick="return confirm('Are you sure <?php echo $name; ?> job is completed and is to be dilivered')">JOB HAVE BEEN COMPLETED , DELIVER IT TO THE CUSTOMER</button>
      </div>
    </div>
  </form>
