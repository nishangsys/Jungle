...
<body>
   ...
   <?php
	...
	while($rows=mysql_fetch_assoc($runs)){
	  ...
	  $tot_stu="
	 SELECT COUNT(name) as total from students where ayear='$ac_year'  	";
	...
	$tot_male="
	 SELECT COUNT(name) as total from students where ayear='$ac_year' and name='male'	";
	$run=mysql_query($tot_male) or die (mysql_error());
	while($row=mysql_fetch_assoc($run)){
		 $male=$row['total'];
	}
	$tot_female="
	 SELECT COUNT(name) as total from students where ayear='$ac_year'  and name='female'	";
	...
	$ind="
	 SELECT COUNT(name),class as total from students where ayear='$ac_year'  GROUP BY class	";
	...
	$allfin="
	 SELECT SUM(expected) as total from installment where ayear='$ac_year' 	";
	$run=mysql_query($allfin) or die (mysql_error());
	while($row=mysql_fetch_assoc($run)){
		 $money=$row['total'];  
	}
	$allowes="
	 SELECT SUM(expected) as total from finance where ayear='$ac_year' 	";
...
$ind="
	 SELECT COUNT(name),class  from students where ayear='$ac_year'  GROUP BY class order by class ASC	";
	$run=mysql_query($ind) or die (mysql_error());
	$i=1;
	while($row=mysql_fetch_assoc($run)){
...
$ind="
	 SELECT SUM(newin),date  from finance where ayear='$ac_year'  GROUP BY newin order by newin ASC	";
	$run=mysql_query($ind) or die (mysql_error());
	$i=1;
	while($row=mysql_fetch_assoc($run)){
...
$allowe="
	 SELECT SUM(expected),date from finance where ayear='$ac_year' GROUP BY date 	";
...
$all="SELECT SUM(expected),SUM(paid) from finance where ayear='$ac_year' ";
