...
define ("DB_NAME","super_db"); // set database name
 ...
 $db_basket=$ybasket;
	...
	?>
     ...
     <div style="width:98%; height:auto; overflow:hidden; padding-bottom:20px; float:right; border:3px dashed#FF0; outline:1px dashed#000; background:#fff; "  >
        ...
        <A href="index.php?command=<?php echo $table; ?>&area=<?php echo  $section; ?>&tabs=<?php echo $table; ?>" style="color:#fff" onclick="return confirm('<?php echo $_SESSION['user_name']; ?> Are you Sure ')">
      ...
      COMMAND
       ...
       TABLE <?PHP echo $_GET['tabs']; ?> 
            ...
            <?PHP
			$a = $con->query("SELECT SUM(qty),SUM(total),price,product,category,ids,id,price,qty,total FROM basket where tab='".$_GET['tabs']."' and status='1' and qty>0 GROUP BY category,product order by product ") or die(mysqli_error($con));
			$i=1;
			?>
  ...
  <a href="index.php?add=<?php echo $rows['id']; ?>&area=<?php echo  $section; ?>&tabs=<?php echo $table; ?>&temp=<?php echo $rows['category']; ?>">
...
<a href="index.php?reduce=<?php echo $rows['id']; ?>&area=<?php echo  $section; ?>&tabs=<?php echo $table; ?>&temp=<?php echo $_GET['temp']; ?>&temp=<?php echo $rows['category']; ?>">
 ...
 <!--------TOTAL---->   
   <div class="col-sm-12" style="background:#003; padding:0px 0px; text-align:CENTER; border:1px solid#fff">
     <span style="background:#090; padding:15px 20px; float:right; font-size:18px; color:#fff; font-weight:bold ">  
      <?PHP
      	$a = $con->query("SELECT SUM(total) as totbill FROM basket where tab='".$_GET['tabs']."' and status='1' GROUP BY tab") or die(mysqli_error($con));
